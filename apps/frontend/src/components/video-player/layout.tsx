...
export function VideoLayout({ thumbnails }: VideoLayoutProps) {
  return (
    <>
      ...
      /> */}
      <Controls.Root className="media-controls:opacity-100 absolute bottom-0 z-10 -mt-[82px] flex h-fit w-full flex-col bg-gradient-to-t from-black/30 via-black/15 to-transparent opacity-0 transition-opacity">
        <Tooltip.Provider>
          <div className="flex-1 flex-row" />
          <Controls.Group className="w-full flex-row items-center px-2">
            <Sliders.Time thumbnails={thumbnails} />
          </Controls.Group>
          <Controls.Group className="-mt-0.5 flex w-full flex-row items-center px-2 pb-2">
