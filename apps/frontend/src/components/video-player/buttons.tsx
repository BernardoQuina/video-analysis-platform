    ...
    <Tooltip.Root>
      <Tooltip.Trigger asChild>
        <PlayButton className={buttonClass}>
          {isPaused ? (
            <PlayIcon className="h-7 w-7 translate-x-px fill-current stroke-1" />
          ) : (
            <PauseIcon className="h-7 w-7 fill-current stroke-0" />
          )}
        </PlayButton>
      </Tooltip.Trigger>
      <Tooltip.Content
    ...
    <Tooltip.Root>
      <Tooltip.Trigger asChild>
        <MuteButton className={buttonClass}>
          {isMuted || volume == 0 ? (
            <MuteIcon className="h-7 w-7 stroke-[1.75]" />
          ) : volume < 0.5 ? (
            <VolumeLowIcon className="h-7 w-7 stroke-[1.75]" />
          ) : (
            <VolumeHighIcon className="h-7 w-7 stroke-[1.75]" />
          )}
        </MuteButton>
      </Tooltip.Trigger>
      <Tooltip.Content
    ...
    <Tooltip.Root>
      <Tooltip.Trigger asChild>
        <PIPButton className={buttonClass}>
          {isActive ? (
            <PictureInPictureExitIcon className="h-7 w-7 stroke-[1.75]" />
          ) : (
            <PictureInPictureIcon className="h-7 w-7 stroke-[1.75]" />
          )}
        </PIPButton>
      </Tooltip.Trigger>
      <Tooltip.Content
    ...
    <Tooltip.Root>
      <Tooltip.Trigger asChild>
        <FullscreenButton className={buttonClass}>
          {isActive ? (
            <FullscreenExitIcon className="h-7 w-7 stroke-[1.75]" />
          ) : (
            <FullscreenIcon className="h-7 w-7 stroke-[1.75]" />
          )}
        </FullscreenButton>
      </Tooltip.Trigger>
      <Tooltip.Content
